...
pub fn gen_step_directions() -> Vec<Vec<i32>> {
    let mut steps: Vec<Vec<i32>> = vec![];
    for n in STEP_DIRECTIONS {
...
impl<'a> Strategy<'a> {
    pub fn new(mut state: GameState, directions: &'a Vec<Vec<i32>>) -> Self {
    ...
    fn best_move(&self, mut me: Player, mut items: Vec<bool>, depth: usize) -> Move {
        ...
        let best_move = self
            ...
            .map(|dirs| {
                // for return value
                let angle_0 =
                    angle_by_index_semiforward(dirs[0], STEP_DIRECTIONS[0], 0.9) + angle(&me.speed);
                let target = Point {
                    x: me.pos.x + (ACC * f32::sin(angle_0)) as i32,
                    y: me.pos.y + (ACC * f32::cos(angle_0)) as i32,
                };
                let mut score_inc = vec![0.0; dirs.len()];
                let mut player = me.clone();
                let mut items = items.clone();
                for (d, i) in dirs.into_iter().enumerate() {
                    let angle = angle_by_index_semiforward(*i, STEP_DIRECTIONS[d], 0.9)
                        + angle(&player.speed);
                    player.target = Point {
                        x: player.pos.x + (ACC * f32::sin(angle)) as i32,
                        y: player.pos.y + (ACC * f32::cos(angle)) as i32,
                    };
                    let score_inc_tmp = self.precompute.step(&mut player, &mut items, depth);
                    score_inc[d] = score_inc_tmp;
                }
                let Move { mut score, .. } = self.best_move(player, items, STEP_DIRECTIONS.len());
                for inc in score_inc.into_iter().rev() {
                    score = decay_f32(inc, score);
                }
                Move {
                    score: score,
                    target: target,
            ...
            .max_by_key(|mv| (mv.score * 1000000f32) as i32)
            .unwrap();
